...
class State:
    ...
    def save_object(self, class_name, m, i, x, y, angle):
        delta_same_object = 5
        bbox = self.get_size_data(m, i)
        size = abs(bbox[1] - bbox[3])
        if not self.use_robot_location:
            x, y = get_coordinates(x, y, angle, bbox, size)
        if class_name in self.objects.keys():
            if m['timestamp'] - self.objects[class_name][-1]["last_timestamp"] > delta_same_object:
                # add new object
                self.objects[class_name][-1].append(len(self.images))
                self.objects[class_name].append({
                        "x": x,
                        "y": y,
                        "size": size,
                        "last_timestamp": m['timestamp'],
                        "data_indexes": self.objects[class_name][-1]["data_indexes"],
                        "count": 1
                    })  # new object
            else:
                ...
                if size > self.objects[class_name][-1]["size"]:
                    ...
                    self.objects[class_name][-1] = {
                        ...
                        "last_timestamp": m['timestamp'],
                        "data_indexes": self.objects[class_name][-1]["data_indexes"],
                        "count": self.objects[class_name][-1]["count"] + 1
                    }  # overwrite current object data
                else:
                    self.objects[class_name][-1]['last_timestamp'] = m['timestamp']
        elif class_name in self.given_object_data.keys() or self.simulated:
            # add to object map
    ...
    def detect_cb_pose(self, m):
        ...
        if self.simulated:
            for i in range(len(m['object']['payload']['objects']['detections'])):
                ...
                # robot pose
                print(m)
                # comment to non-zeros if you have something
                x = 0#m['pose']['payload']['pose']['pose']['position']['x']
                y = 0#m['pose']['payload']['pose']['pose']['position']['y']
                angle = 0#m["payload"]["pose"]["orientation"]["z"]
                self.save_object(class_name, m, i, x, y, angle)
        else:
    ...
    def submit(self):
            ...
            for obj_name in self.objects.keys():
                for obj in self.objects[obj_name]:
                    if obj["count"] > self.min_count:
                        if obj_name in self.given_object_data.keys():
        ...
        for name in self.given_object_data.keys():
